...
---
> Codex compatibility: migrated from `.claude/skills/qa-plan/SKILL.md`. Original Claude-specific metadata
...
> `{"user-invocable": "true", "allowed-tools": "Read, Glob, Grep, Write, AskUserQuestion", "agent": "qa-lead"}`.
>
> Invocation in this template uses `$qa-plan`. If structured user input
