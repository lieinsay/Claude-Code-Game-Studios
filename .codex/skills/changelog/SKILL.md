...
---
> Codex compatibility: migrated from `.claude/skills/changelog/SKILL.md`. Original Claude-specific metadata
...
> `{"user-invocable": "true", "allowed-tools": "Read, Glob, Grep, Bash, Write", "context": "|", "!git tag --list --sort=-v": "refname 2>/dev/null | head -5", "model": "haiku"}`.
>
> Invocation in this template uses `$changelog`. If structured user input
