...
public sealed class UIManager
{
    ...
    public bool PressTab(bool shift = false)
    {
        ...
        var focusChain = FocusChainForPanel(modalPanel);
    ...
    private void BindHubHud(Dictionary<string, string> fields, List<string> queryNames)
    {
        ...
        fields["cargo_display"] = cargo is null ? MissingResourceDisplay : $"{ReadString(cargo, "current")}/{ReadString(cargo, "max")}";
    ...
    {
        KeyboardFocusElementId = focusRestoreStack.Count > 0 ? focusRestoreStack.Pop() : string.Empty;
    }
    ...
    private static string FirstFocusableElement(string panelId)
    {
        var focusChain = FocusChainForPanel(panelId);
        return focusChain.Length > 0 ? focusChain[0] : string.Empty;
    }
    ...
    private bool IsElementInActiveFocusScope(string elementId)
    {
        if (IsModalOpen())
        {
            return FocusChainForPanel(modalPanel).Contains(elementId, StringComparer.Ordinal);
        }

        foreach (var panelId in nonModalPanels.AsEnumerable().Reverse())
        {
            if (FocusChainForPanel(panelId).Contains(elementId, StringComparer.Ordinal))
            {
                return true;
            }
        }
