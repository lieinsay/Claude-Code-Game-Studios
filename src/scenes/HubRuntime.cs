...
public partial class HubRuntime : Node2D
{
	...
	private static readonly Rect2 VoyageOpenWorldBounds = new(new Vector2(172, 382), new Vector2(930, 144));
	...
	private const string VoyageOpenWorldScenePath = "res://src/scenes/voyage/VoyageOpenWorldScene.tscn";
	private const string MistLampWreckScenePath = "res://src/scenes/mist/MistLampWreckScene.tscn";
	...
	private VoyageOpenWorldScene? voyageOpenWorldAsset;
	private MistLampWreckScene? mistLampWreckAsset;
	...
	public override void _UnhandledInput(InputEvent @event)
	{
		...
		if (currentScreen == "exploration" || IsOchreScreen(currentScreen))
		{
	...
	public void OnDepartPressed()
	{
		...
		if (selectedRoute == "route.ochre" || domain.Snapshot.EncounterDestinationId == "location.ochre-island")
		{
			EnterOchreIslandRoute();
			UpdateOnboardingHint();
			return;
		}
	...
	{
		if (!IsOchreScreen(currentScreen))
		{
			return;
		}
		...
		{
			SetFooter(currentScreen == "ochre_island"
				? "条带状铁矿已采集：Resources carried pool 已保留该奖励。"
				: "条带状铁矿已采集：开发入口保留状态可读性，不写入正式 Resources 奖励。");
			return;
		}

		if (currentScreen == "ochre_island" && !domain.HarvestOchreOre())
		{
			SetFooter($"条带状铁矿采集失败：{domain.Snapshot.LastStatus}");
			return;
		}
		...
		SetFooter(currentScreen == "ochre_island"
			? "条带状铁矿采集完成：resource.banded_iron_ore x1 已写入 ResourcesManager carried pool。"
			: "条带状铁矿采集完成：已验证世界锚点和采集后状态；正式资源奖励仍待后续 runtime 集成。");
	}
	...
	public void OnOchreReturnPressed()
	{
		...
		var formalRoute = currentScreen == "ochre_island";
		if (formalRoute)
		{
			domain.ReturnToHub();
		}
		...
		ShowHub();
		SetFooter(formalRoute
			? "已从赭石岛返航至初始岛屿码头；条带状铁矿已结算入飞艇仓库。"
			: "已从赭石岛开发入口返回 Hub；正式 playable route 未被替换。");
	}
	...
	public void OnLoadPressed()
	{
		...
		else if (IsOchreScreen(currentScreen))
		{
			ShowOchreSurface();
			SetSaveStatus($"加载完成：本地航行日志 gen {result.Generation} / {restoredScreenName}");
		}
	...
	/// <summary>Returns independent voyage open-world scene evidence for smoke tests and QA diagnostics.</summary>
	public Godot.Collections.Dictionary DebugVoyageOpenWorldAssetEvidence()
	{
		return voyageOpenWorldAsset?.DebugSceneAssetEvidence() ?? new Godot.Collections.Dictionary
		{
			["scene_id"] = "voyage_open_world_scene",
			["runtime_contract_id"] = "voyage_open_world_scene",
			["route_id"] = "route.mist",
			["destination_id"] = "location.mist-short",
			["arrival_scene_id"] = "mist_lamp_wreck_scene",
			["arrival_runtime_contract_id"] = "exploration_mist_island",
			["world_layer_ready"] = false,
			["takeoff_transition_ready"] = false,
			["active_driving_view_ready"] = false,
			["route_corridor_ready"] = false,
			["fog_problem_ready"] = false,
			["wreckage_problem_ready"] = false,
			["bird_evasion_ready"] = false,
			["destination_silhouette_ready"] = false,
			["retreat_anchor_ready"] = false,
			["ui_evidence_allowed_for_scene"] = false,
		};
	}

	/// <summary>Returns independent mist-lamp wreck scene evidence for smoke tests and QA diagnostics.</summary>
	public Godot.Collections.Dictionary DebugMistLampWreckAssetEvidence()
	{
		return mistLampWreckAsset?.DebugSceneAssetEvidence() ?? new Godot.Collections.Dictionary
		{
			["scene_id"] = "mist_lamp_wreck_scene",
			["runtime_contract_id"] = "exploration_mist_island",
			["return_target_scene_id"] = "initial_island_scene",
			["return_target_runtime_contract_id"] = "hub_island_dock",
			["world_layer_ready"] = false,
			["player_spawn_ready"] = false,
			["island_mass_ready"] = false,
			["search_wreck_ready"] = false,
			["search_anchor_ready"] = false,
			["return_ship_ready"] = false,
			["return_helm_anchor_ready"] = false,
			["return_takeoff_ready"] = false,
			["water_boundary_ready"] = false,
			["island_has_threat_zone"] = false,
			["ui_evidence_allowed_for_scene"] = false,
		};
	}
	...
	public Godot.Collections.Dictionary DebugScenePhysicsContract(string sceneId)
	{
		return sceneId switch
		{
			...
			"exploration_mist_island" => BuildScenePhysicsContract(
				...
				"ground plane supports up/down/left/right movement; mast, beacon beam, and return takeoff trail are height_only cues with ground-shadow/landing-height readability, not extra walkable floors",
				"primary_walkable_layer=mist_wreck_ground_01; walkable_layer: shoreline_path, search_wreck_approach, return_ship_approach; transition_layer: return_helm_takeoff_to_voyage_and_hub; height_only_layer: mast, beacon_beam, return_takeoff_trail; blocked_layer: sea, cliff_edge, wreck_body, return_ship_hull; visual_layer: mist_horizon",
				...
				"N/A true: single exterior island floor; floor_id=mist_wreck_ground_01; floor_index=0; is_active_floor=true; visibility_mode=full_visible; walkable_bounds=ExplorationWalkBounds; vertical_connectors=return_helm_takeoff_to_voyage_and_hub; occluders_hidden_above=none; interactions_enabled=search_scan_arc,return_helm",
				"mist_wreck_ground_01",
				"mist_wreck_ground_01",
				0,
				true,
				"full_visible",
				"return_helm_takeoff_to_voyage_and_hub",
				"none",
				...
				"z_world_background < z_island_body_path < z_wreck_return_ship_units < z_interaction_markers",
				"blocking_static: island edge, sea, cliff edge, return ship hull, search wreck body; soft_overlap: search scan arc, return helm; height_marker: mast, beacon beam, return takeoff trail",
				"water: blocking_static hazard boundary; glass: none; mirror: none; elastic: none; pushable: none in current slice",
			...
			"voyage_open_world_scene" => BuildScenePhysicsContract(
				...
				"ground plane supports up/down/left/right driving readability across an air-lane corridor; ship bow and cockpit frame are height_only foreground cues, not UI or extra walkable floors",
				"primary_walkable_layer=voyage_air_lane_01; walkable_layer: route_corridor, beacon_chain_lane; transition_layer: takeoff_trail_from_hub_dock, arrival_lane_to_exploration_mist_island, retreat_beacon_to_hub_dock; height_only_layer: ship_bow_foreground, cockpit_window_frame, bird_silhouette; blocked_layer: fog_bank_core, wreckage_field, deep_cloudsea_edge; visual_layer: far_destination_silhouette, cloud_parallax",
				"N/A true for floor_cutaway/interior_instance; voyage foreground occluders are cockpit/bow readability frames and never hide a passable behind-object route; behind_object_reveal=N/A true while collision identity remains on fog, wreckage, and retreat anchors",
				"N/A true: single air-lane floor; floor_id=voyage_air_lane_01; floor_index=0; is_active_floor=true; visibility_mode=full_visible; walkable_bounds=VoyageOpenWorldBounds; vertical_connectors=takeoff_trail,arrival_lane,retreat_beacon; occluders_hidden_above=none; interactions_enabled=heading,throttle,scan,evade,retreat",
				"voyage_air_lane_01",
				"voyage_air_lane_01",
				0,
				true,
				"full_visible",
				"takeoff_trail,arrival_lane,retreat_beacon",
				"none",
				"heading,throttle,scan,evade,retreat",
				"N/A true: cockpit foreground and cloud layers do not hide a passable behind-object route; fog and wreckage preserve collision/avoidance identity",
				"2.2m player height = 28px marker; ship bow spans cockpit scale; beacon gaps and wreckage passages remain readable against player_unit",
				"z_sky_backdrop < z_cloud_parallax < z_route_corridor < z_risk_units < z_ship_foreground < z_interaction_markers",
				"blocking_static: fog bank core, wreckage field, deep cloudsea edge; soft_overlap: route corridor, beacon chain, retreat beacon, scan window; height_marker: ship bow foreground, cockpit window frame, bird silhouette",
				"fog=gameplay_affecting slow/scan risk window; cloudsea=blocking_static hazard boundary; wreckage=blocking_static avoidance field; bird_shadow=visual_only ecological warning until #10 owns consequences",
				"Clamp ship/player proxy into VoyageOpenWorldBounds; retreat beacon remains soft_overlap so failed driving can recover to hub_island_dock",
				8),
	...
	private static Godot.Collections.Array<Godot.Collections.Dictionary> BuildPhysicalBehaviorCatalog(string sceneId)
	{
		return sceneId switch
		{
			...
			{
				BuildPhysicalBehavior("mist_water_boundary", "hazardous_boundary", "hazardous + water_deep + blocking_static", "deep water/void boundary; blocks ground units; no island-local threat or damage source in current slice", "shoreline, cliff, and sea feedback", "player_unit, pushable_unit", 80, "hazardous boundary resolves before water visual and before trigger_only", "clamp player back into ExplorationWalkBounds", "world_playable_scene", false),
				BuildPhysicalBehavior("mist_search_scan_anchor", "trigger_only", "trigger_only + soft_overlap", "three-step scan anchor; no entity collision; requires proximity and Use dispatch", "scan calibration, echo lock, salvage pulse feedback", "player_unit", 45, "trigger_only is ignored until proximity and Use gate pass", "reset scan stage by moving away or returning to Hub; no forced stuck state", "world_playable_scene", false),
				BuildPhysicalBehavior("mist_return_helm_anchor", "trigger_only", "trigger_only + soft_overlap", "two-step return anchor; no entity collision; requires proximity and Use dispatch", "engine preheat, takeoff trail, and piloting prompt", "player_unit", 40, "trigger_only cannot override hazardous boundary clamp", "return takeoff leads to voyage/hub flow after preheat", "world_playable_scene", false),
			...
			{
				BuildPhysicalBehavior("voyage_fog_bank", "gameplay_affecting_fog", "fog + scan_window + blocking_static", "first-route fog band; scan/slow/readability consequences stay with #10 when live driving is wired", "fog bank covers destination briefly while beacon chain remains readable", "ship_proxy, player_unit", 75, "fog risk wins over visual cloud parallax and route hints", "retreat beacon or corridor clamp restores a safe heading", "world_playable_scene", false),
				BuildPhysicalBehavior("voyage_wreckage_field", "avoidance_field", "blocking_static + avoidance_gap", "wreckage creates readable gaps and collision risk without becoming a UI prompt", "near and far wreckage silhouettes bracket a passable gap", "ship_proxy", 65, "avoidance_field beats route corridor but loses to explicit retreat anchor", "clamp ship proxy back to nearest route corridor gap", "world_playable_scene", false),
				BuildPhysicalBehavior("voyage_retreat_beacon", "trigger_only", "trigger_only + soft_overlap", "spatial retreat anchor; no collision authority; allows route reset or hub return when #10 owns the live state", "warm retreat lamp visible behind ship path", "ship_proxy, player_unit", 45, "trigger_only cannot override fog or wreckage collision", "return to hub_island_dock or last safe heading", "world_playable_scene", false),
				BuildPhysicalBehavior("voyage_bird_silhouette", "visual_only_ecological_warning", "bird_shadow + visual_only", "large bird proves ecological pressure readability but does not own damage in this asset pass", "distant bird silhouette and wing-shadow direction", "player_unit readability only", 15, "visual_only loses to fog, wreckage, and retreat behavior", "no stuck state possible; ignore for movement", "world_playable_scene", false),
			},
	...
	public Godot.Collections.Dictionary DebugDomainSnapshot()
	{
		...
		return new Godot.Collections.Dictionary
		{
			...
			["ochre_ore_in_storage"] = snapshot.OchreOreInStorage,
			["ochre_ore_carried"] = snapshot.OchreOreCarried,
			["ochre_ore_harvested"] = ochreOreHarvested,
	...
	private void AddChartGreyboxSet()
	{
		...
		AddScenePolygon(chartSceneItems, "ChartRouteOchreLine",
			...
			new Color(0.64f, 0.44f, 0.28f, 0.92f));
		...
		AddSceneEllipse(chartSceneItems, "ChartOchreDestinationNode", new Vector2(704, 422), new Vector2(34, 22), new Color(0.70f, 0.44f, 0.26f, 0.98f));
		...
		chartOchreSelectionFrame = AddSceneRect(chartSceneItems, "ChartRouteOchreSelectionFrame", new Vector2(652, 394), new Vector2(112, 64), new Color(0.84f, 0.62f, 0.38f, 0.38f));
		...
		AddSceneLabel(chartSceneItems, "ChartOchreRouteLabel", new Vector2(652, 454), new Vector2(180, 24), "赭石岛航线 / 铁矿");
	...
	{
		AddVoyageOpenWorldSceneAsset();
		AddMistLampWreckSceneAsset();
	...
	private void AddVoyageOpenWorldSceneAsset()
	{
		var scene = ResourceLoader.Load<PackedScene>(VoyageOpenWorldScenePath);
		if (scene?.Instantiate() is not VoyageOpenWorldScene voyageScene)
		{
			AddSceneRect(explorationSceneItems, "VoyageOpenWorldAssetLoadFailure", new Vector2(284, 246), new Vector2(640, 44), new Color(0.46f, 0.16f, 0.16f, 0.95f));
			AddSceneLabel(explorationSceneItems, "VoyageOpenWorldAssetLoadFailureLabel", new Vector2(298, 254), new Vector2(610, 26), "voyage_open_world_scene 独立场景加载失败");
			return;
		}

		voyageOpenWorldAsset = voyageScene;
		voyageScene.Name = "VoyageOpenWorldSceneRuntimeInstance";
		voyageScene.Visible = false;
		sceneLayer?.AddChild(voyageScene);
		explorationSceneItems.Add(voyageScene);
	}

	private void AddMistLampWreckSceneAsset()
	{
		var scene = ResourceLoader.Load<PackedScene>(MistLampWreckScenePath);
		if (scene?.Instantiate() is not MistLampWreckScene mistScene)
		{
			AddSceneRect(explorationSceneItems, "MistLampWreckAssetLoadFailure", new Vector2(284, 300), new Vector2(640, 44), new Color(0.46f, 0.16f, 0.16f, 0.95f));
			AddSceneLabel(explorationSceneItems, "MistLampWreckAssetLoadFailureLabel", new Vector2(298, 308), new Vector2(610, 26), "mist_lamp_wreck_scene 独立场景加载失败");
			return;
		}

		mistLampWreckAsset = mistScene;
		mistScene.Name = "MistLampWreckSceneRuntimeInstance";
		mistScene.Visible = false;
		sceneLayer?.AddChild(mistScene);
		explorationSceneItems.Add(mistScene);
	}
	...
	private void AddOchreGreyboxSet()
	{
		...
		ochreOreSemanticLabel = AddSceneLabel(ochreSceneItems, "OchreOreSemanticLabel", new Vector2(520, 454), new Vector2(300, 24), "矿脉：可采集 / Resources ready");
	...
	private void EnterOchreIslandRoute()
	{
		currentScreen = "ochre_island";
		ochreOreHarvested = domain.Snapshot.OchreOreCarried + domain.Snapshot.OchreOreInStorage > 0;
		ochreReturnPrepStage = 0;
		explorationStep = 0;
		playerPosition = OchreIslandPlayerStart;
		ShowOchreSurface();
	...
	private void ShowOchreSurface()
	{
		SetHubControlsEnabled(false);
		SetWorldMode(currentScreen);
		UpdateOchreSceneSemantics();
		SetFooter(currentScreen == "ochre_island"
			? "赭石岛正式航线：移动到条带状铁矿按 E 采集，或到返航点按 E 两步返回初始岛屿。"
			: "赭石岛开发入口：移动到条带状铁矿按 E 采集，或到返航点按 E 两步返回 Hub。");
	}
	...
	private void UpdateOchreSceneSemantics()
	{
		if (ochreOreSemanticLabel is not null)
		{
			ochreOreSemanticLabel.Text = ochreOreHarvested
				? currentScreen == "ochre_island" ? "矿脉：已采集 / carried resource.banded_iron_ore" : "矿脉：已采集 / debug state"
				: currentScreen == "ochre_island" ? "矿脉：可采集 / Resources ready" : "矿脉：可采集 / debug state";
		}
	...
	private void SetWorldMode(string mode)
	{
		...
		SetSceneGroupVisible(ochreSceneItems, IsOchreScreen(mode));
		if (mode == "hub")
		{
		...
		if (ochreOreMarker is not null) ochreOreMarker.Visible = IsOchreScreen(mode);
		if (ochreReturnMarker is not null) ochreReturnMarker.Visible = IsOchreScreen(mode);
		if (playerMarker is not null)
		{
			if (mode == "chart")
			{
		...
		else if (IsOchreScreen(currentScreen))
		{
			...
			if (oreDistance <= InteractionRadius && oreDistance <= returnDistance)
			{
				...
				prompt = ochreOreHarvested
					? currentScreen == "ochre_island" ? "条带状铁矿已采集：Resources carried pool 已记录。" : "条带状铁矿已采集：开发入口只验证状态。"
					: currentScreen == "ochre_island" ? "按 E 采集条带状铁矿：写入 ResourcesManager。" : "按 E 采集条带状铁矿：验证世界资源锚点。";
			}
			else if (returnDistance <= InteractionRadius)
			{
				...
				prompt = ochreReturnPrepStage <= 0
					? "按 E 预热赭石岛返航锚点。"
					: currentScreen == "ochre_island" ? "按 E 从赭石岛正式返航初始岛屿。" : "按 E 从赭石岛开发入口返回 Hub。";
			}
		}
	...
	private static bool IsOchreScreen(string screen) =>
		screen is "ochre_island" or "ochre_dev";
