...
| --- | --- | --- | --- | --- | --- | --- |
| `initial_island_scene` | 初始岛屿 | 新游戏开始 / 返回原点 | `asset-reset-required` | `hub_island_dock` 旧运行时合同可作为技术草稿参考，但 `src/presentation/playable_slice_authored_content.json` 中旧原型 / 摆放实例已撤销；不得作为现存资产证据。 | `production/scene-specs/initial-island-scene.md`; 历史 Polish 015 证据仅供问题背景参考 | 从创建适合性记录、Godot asset contract / review / execute 重新实现。 |
