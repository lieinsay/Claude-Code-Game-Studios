...
| `ochre_island_scene` | 赭石岛 | `S4_chart` 选择 `route.ochre` 后抵达 | `implemented` | 独立 Godot 资产、条带状铁矿原型、6 个作者化场景单位、#20 运行时合同、正式 `route.ochre`、Resources 奖励写入和 Hub 返航结算均已链接。Debug build 入口只作开发诊断，不作为 production-ready 场景证据。 | `production/scene-specs/ochre-island-scene.md`; `.godot-ai/verification/composite-feature/ochre_island_formal_route.verification.md`; `production/qa/evidence/ochre-island-godot-asset-execution-evidence.md`; `production/unit-specs/fixed-scene-objects/banded-iron-ore.md`; `design/gdd/resources-goods-capacity.md` | 当前是生产可追踪灰盒；非 headless 截图、最终美术 / 音频和完整实时航行表现仍是后续风险。 |
| `old_market_edge_scene` | 旧集市边缘 | 后续市场内容候选 | `spec_drafted` | 独立 Godot asset-gate 已建立：`src/scenes/market/OldMarketEdgeScene.tscn`、market plaza / path / open stall / closed stall / notice board / cloudsea boundary 作者化单位和 `HubRuntime.DebugScenePhysicsContract("old_market_edge_scene")`。当前不暴露 `route.market`，也不把 `S9_market` 或 Chart 控件当作场景证据。 | `production/scene-specs/old-market-edge-scene.md`; `.godot-ai/verification/scene/old_market_edge_scene.verification.md`; `design/gdd/port-village-market.md`; `design/gdd/scene-composition-system.md` | 后续市场阶段接入正式 route、SettlementManager 世界交互、S9 购买 UI、截图包和最终美术 / 音频。 |
