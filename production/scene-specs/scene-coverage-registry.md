...
| `ship_interior_layered` | 云织号船内分层水平场景 | Boarding from initial island / return from voyage | `covered-by-current-note` | Historical runtime contract exists under `hub_ship_interior`, but design authority now treats this as a horizontal layered scene requiring Layer / Height Model, Cutaway / Reveal, and behind-object reveal | `design/gdd/airship-hub.md`; `production/polish-backlog/story-polish-015-island-ship-interior-and-search-gameplay-design.md`; `production/qa/evidence/scene-physics-layer-cutaway-floor-state-evidence.md`; `production/qa/evidence/scene-physics-unit-catalog-evidence.md` | Extract standalone ship-interior spec and update #20 contract classification from vertical-only assumptions to horizontal-layered design. |
| `voyage_open_world_scene` | 航行大场景 | Departure from initial island to demo destinations | `tracked-gap` | #20 contract required before implementation readiness: pseudo-3D camera-aligned flight, route boundaries, risk objects, cloud/fog special surfaces, recovery rules | `design/gdd/navigation-route-risk.md`; `design/gdd/scene-composition-system.md` | Draft independent voyage scene spec before readability review; merge the two current demo routes into this scene. |
