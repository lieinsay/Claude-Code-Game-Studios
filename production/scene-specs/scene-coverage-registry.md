...
| `chart_table_scene` | Chart table surface | Ship interior helm anchor | `covered-by-current-note` | Not currently a separate #20 playable scene contract; Story 003 classifies it as an authored UI-assisted surface anchored inside `hub_ship_interior` until standalone physical scene play is required | `tests/smoke/session_shell_visual_probe.gd`; `design/gdd/scene-composition-system.md`; `production/qa/evidence/polish-015-island-ship-interior-and-search-gameplay-evidence.md`; `production/scene-specs/scene-vs-ui-evidence-boundary.md` | Do not count Chart buttons/route UI as scene evidence. If Chart becomes a separate enterable physical scene, draft its own #20 contract first. |
