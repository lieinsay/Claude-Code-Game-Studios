...
UI 自身也必须接受严格设计。任何常驻 HUD、锚点面板、模态、半模态覆盖层、全屏 UI、toast / hint 或 debug overlay，都必须按 `production/ui-specs/` 声明分类、打开方式、绑定对象、显示优先级和输入 / 焦点影响。UI 规格只能证明 UI 合格，不能证明场景或场景单位合格。
...
| HUD 标签、状态面板、航线文字、保存 / 读取文字、新手提示、按钮、菜单、模态面板、调试标签、smoke-only 覆盖文本 | 辅助文字、可访问性支持、焦点路由证明、调试诊断 | 物理场景单位、场景身份节点、交互锚点、视口身份、物理合同证明或世界场景证据替代 |

## UI 规格门禁

| UI 类型 | 必须证明 | 阻塞条件 |
| --- | --- | --- |
| 常驻 HUD | 场景 / 阶段门控、最大屏幕占比、默认不抢焦点、不吞世界输入 | 无条件跨场景常驻；遮挡世界身份或核心锚点 |
| 绑定面板 | 绑定世界锚点、场景单位、系统状态或明确快捷键 | 只从空白 UI 按钮打开，场景里没有对应对象 |
| 模态 / 半模态 | 阻断哪些输入、保留哪些输入、初始焦点、关闭 / 恢复规则 | 焦点穿透、Esc 语义不清、关闭后上下文丢失 |
| 全屏 UI 表面 | 进入 / 离开路径、底层世界状态处理、输入接管规则 | 看似场景但没有世界锚点，或无法返回 |
| toast / hint | 自动消失、不抢焦点、不承载关键决策 | 用短提示替代关键操作或场景对象 |
| debug overlay | 仅开发 / QA 可见，不计入玩家体验证据 | 出现在玩家验收路径，或被当作场景证据 |
...
| `old_market_edge_scene` | 后续市场内容候选，不属于当前 demo 第二岛屿；已有 asset-gate 独立场景证据 | 市场 UI 不能计入；后续进入 demo 时仍需正式 route、SettlementManager 世界交互、摊位 / NPC 锚点、可通行性和 #20 合同证据 |
