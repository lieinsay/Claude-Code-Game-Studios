...
- Follow-up risk pass: route/search data moved out of code and later advanced to `src/presentation/playable_slice_authored_content.json`; Navigation preflight now queries `ModuleHullManager.CanDepart()` after startup modules are installed through authored content data.
- Windowed visual capture added at `production/qa/evidence/polish-001-windowed-session-shell-hub-probe.png`; smoke also asserts the final screenshot state has no stale return-Hub onboarding hint.
- Remaining Polish boundary: the route/search data is now an authored MVP content slice and greybox spatial markers are still presentation fixtures. They are no longer hardcoded runtime authority, but final route/search scale-up remains downstream Polish scope.
