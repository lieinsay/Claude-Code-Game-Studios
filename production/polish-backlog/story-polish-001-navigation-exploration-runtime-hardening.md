...
- Follow-up risk pass: route/search fixture data moved out of code into `src/presentation/playable_slice_runtime_fixture.json`; Navigation preflight now queries `ModuleHullManager.CanDepart()` after startup modules are installed through the fixture.
...
- Remaining Polish boundary: the route/search data is still an MVP fixture file and greybox spatial markers are still presentation fixtures. They are no longer hardcoded runtime authority, but richer authored content remains downstream Polish scope.
