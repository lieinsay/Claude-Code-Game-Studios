...
| TC-UIHUD-002 | Entry shell | Use the visible start entry to enter the runtime. Exercise the settings/audio path if available. | Start flow reaches the hub. Audio confirmation remains non-blocking when muted or unavailable. | PASS | `ShellUiTest` 15/15 PASS. Godot runtime probe PASS. |
