# chart_table_scene 用户可读性验收记录

> **父清单**: [scene-composition-user-readability-checklist.md](scene-composition-user-readability-checklist.md)
> **当前状态**: `PENDING_USER_REVIEW_OR_SCOPE_DECISION`
> **Release handoff**: `BLOCKED_FOR_RELEASE`

## Scope 决策

用户需要先决定：当前 `chart_table_scene` 作为 ship-interior 内的 UI-assisted surface 是否可以进入 ship-interior handoff，还是必须作为独立 scene spec 重新准备。

| 字段 | 填写值 |
| --- | --- |
| Scope verdict | PENDING / ACCEPT_WITHIN_SHIP_INTERIOR / REQUIRES_STANDALONE_SCENE_SPEC |
| 决策理由 |  |
| 决策者 |  |
| 决策日期 |  |
