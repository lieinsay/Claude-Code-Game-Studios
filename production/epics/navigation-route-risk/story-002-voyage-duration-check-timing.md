...
- [x] **AC-1**: GIVEN distance=short + hull=intact + 无遭遇效果，WHEN T_voyage，THEN = 60/1.0 + 0 + 0 = 60s
- [x] **AC-2**: GIVEN distance=medium + hull=damaged (s=0.9) + 2×gentle_crosswind (各+5s)，WHEN T_voyage，THEN = 120/0.9 + 10 + 0 ≈ 143.3s
- [x] **AC-3**: GIVEN distance=long + hull=critical (s=0.75) + 1×turbulence_zone (等效+3s)，WHEN T_voyage，THEN = 180/0.75 + 0 + 3 = 243.0s

### Formula 1 — Dynamic Recalculation on Band Change

- [x] **AC-4**: GIVEN hull=intact (s=1.0) + T_voyage_base=60s，WHEN hull→damaged (s=0.9) mid-voyage，THEN T_voyage 重算 = (T_distance/s_hull_new) + ΣT_flat + ΣT_temp。T_distance=60 → 60/0.9=66.7s (base)。进度不跳回——当前%保持，到达100%时间变长

### Formula 2 — Encounter Check Timing

- [x] **AC-5**: GIVEN T_base=12s + hull=intact (Δ=0)，WHEN T_check，THEN = 12 × 1.0 = 12s
- [x] **AC-6**: GIVEN T_base=12s + hull=damaged (Δ=-0.10)，WHEN T_check，THEN = 12 × 0.9 = 10.8s
- [x] **AC-7**: GIVEN T_base=12s + hull=critical (Δ=-0.20)，WHEN T_check，THEN = 12 × 0.8 = 9.6s

### Formula 2 — Encounter Count

- [x] **AC-8**: GIVEN short + intact → T_voyage_base=60s + T_check=12s，WHEN N_checks，THEN = ⌊60/12⌋ = 5
- [x] **AC-9**: GIVEN medium + damaged → T_voyage_base=133.3s + T_check=10.8s，WHEN N_checks，THEN = ⌊133.3/10.8⌋ = 12（而非 intact 的 10）
- [x] **AC-10**: GIVEN short + intact + 2×gentle_crosswind (T_voyage=70s)，WHEN N_checks，THEN = ⌊60/12⌋ = 5——不受遭遇效果影响（正反馈循环预防）

### N_checks = 0 合法

- [x] **AC-11**: GIVEN T_voyage_base < T_check（极短航线），WHEN N_checks = 0，THEN 航程零遭遇，正常抵达 ARRIVED。这是合法行为，非错误

### Formula 2 — T_check Hard Lower Limit

- [x] **AC-12**: GIVEN wind_shear 多次叠加使 T_check 低于 4s，WHEN 计算，THEN T_check = max(4s, T_calculated)。硬下限防止触发过快

### Timing with Engine Delta

- [x] **AC-13**: GIVEN 航程 IN_PROGRESS，WHEN 桌面窗口切换导致 delta 变大，THEN elapsed_time 正确累积。遭遇按 elapsed_time 触发——恢复后排队结算，不"错过"
- [x] **AC-14**: GIVEN elapsed_time ≥ T_voyage，WHEN 抵达判定，THEN 使用 epsilon=0.01s 容差。浮点比较防止跳过抵达触发
