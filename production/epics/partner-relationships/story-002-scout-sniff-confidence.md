...
- [ ] **AC-1**: GIVEN cat_state=SNIFFING（前一嗅辨动画进行中），WHEN scout_sniff(another_item) 被调用，THEN 返回 {success: false, reaction_id: -1, error: "cat_busy"}。无数据变更
- [ ] **AC-2**: GIVEN cat_state=IDLE_LIVING_QUARTERS 或 BENCH_ADJACENT，WHEN scout_sniff(item)，THEN 状态门控通过——算法进入 Step 1

### Already Sniffed Check (Step 1)

- [ ] **AC-3**: GIVEN item_id 已在 sniffed_items 中，WHEN scout_sniff(item_id)，THEN 返回 {success: false, reaction_id: REACTION_ALREADY_SMELLED}。不调用 reveal_rumor()。不修改 sniffed_items
- [ ] **AC-4**: GIVEN item_id 不在 sniffed_items 中，WHEN scout_sniff(item_id)，THEN Step 1 通过——进入 Step 2

### Signature Reading (Step 2)

- [ ] **AC-5**: GIVEN item.cat_sniff_signature 为 null，WHEN scout_sniff(item_id)，THEN 返回 {success: false, reaction_id: REACTION_CONFUSED}。不调用任何 #6 API。item_id 不加入 sniffed_items
- [ ] **AC-6**: GIVEN item.cat_sniff_signature 存在但 reveal_target="" 或 null，WHEN scout_sniff(item_id)，THEN 返回 REACTION_CONFUSED。与 null 签名同处理（E.2.f 防御性检查）

### F.1 Confidence Clamp (Step 3)

- [ ] **AC-7**: GIVEN raw_confidence=0, 30, 66, 67, 90, 100（6 参数化用例），WHEN _clamp_confidence(raw)，THEN 分别返回 0, 30, 66, 66, 66, 66。min(raw, 66) 恒成立
- [ ] **AC-8**: GIVEN item.cat_sniff_signature 缺失 confidence 字段，WHEN _get_sniff_signature()，THEN confidence 默认 0。不崩溃

### Intel API Calls (Step 4)

- [ ] **AC-9**: GIVEN 有效签名 + 未嗅辨物品，WHEN scout_sniff() 成功路径，THEN IntelManager.reveal_rumor(reveal_target, "partner.sky-cat", [hazard_hint], clamped_confidence) 被调用 1 次
- [ ] **AC-10**: GIVEN 有效签名 + pattern_id=""，WHEN scout_sniff()，THEN report_observation_event() 不被调用。空 pattern_id 静默跳过
- [ ] **AC-11**: GIVEN 有效签名 + pattern_id 非空，WHEN scout_sniff()，THEN IntelManager.report_observation_event(pattern_id, "partner_sniff_success") 被调用 1 次

### State Mutation (Step 5)

- [ ] **AC-12**: GIVEN scout_sniff() 成功，WHEN 完成后，THEN item_id ∈ sniffed_items + sniff_success_occurred=true
- [ ] **AC-13**: GIVEN scout_sniff() 成功 + 之前 sniff_success_occurred 已是 true，WHEN 完成后，THEN sniff_success_occurred 保持 true。不翻转
- [ ] **AC-14**: GIVEN scout_sniff() 成功，WHEN 检查玩家背包，THEN 物品完好——不消费、不移除、不修改数量

### Reaction Selection (Step 6)

- [ ] **AC-15**: GIVEN clamped_confidence ≥ 50，WHEN 选择反应动画，THEN reaction_id=REACTION_CIRCLES_TWICE（绕圈两圈后离开）
- [ ] **AC-16**: GIVEN clamped_confidence < 50 + > 0，WHEN 选择反应动画，THEN reaction_id=REACTION_RUBS_FACE（蹭脸后原地坐下）

### Sniffable Items Filter

- [ ] **AC-17**: GIVEN 玩家背包有 [item_A (有签名), item_B (无签名), item_C (有签名)]，WHEN get_sniffable_items()，THEN 返回 [item_A, item_C]。无签名的 item_B 被过滤
- [ ] **AC-18**: GIVEN 玩家背包为空，WHEN get_sniffable_items()，THEN 返回 []。空状态
