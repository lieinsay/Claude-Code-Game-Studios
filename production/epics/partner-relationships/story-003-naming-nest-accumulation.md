...
- [x] **AC-1**: GIVEN sniff_success_occurred=true + naming_state=PENDING + naming_skip_count=0 + player_returned_to_hub 触发，WHEN _is_naming_eligible()，THEN 返回 true
- [x] **AC-2**: GIVEN sniff_success_occurred=false（所有条件相同），WHEN _is_naming_eligible()，THEN 返回 false。命名永不被触发——猫未证明自己
- [x] **AC-3**: GIVEN naming_state=PROMPTED，WHEN _is_naming_eligible()，THEN 返回 false。不重复提示
- [x] **AC-4**: GIVEN naming_state=COMPLETED，WHEN _is_naming_eligible()，THEN 返回 false。终态
- [x] **AC-5**: GIVEN naming_skip_count=3，WHEN _is_naming_eligible()，THEN 返回 false。窗口已关闭

### Naming State Machine

- [x] **AC-6**: GIVEN 新游戏 + sniff_success_occurred=false，WHEN 初始化，THEN naming_state=PENDING + name="" + naming_done=false + naming_skip_count=0
- [x] **AC-7**: GIVEN 首次成功嗅辨后 + 下次 player_returned_to_hub，WHEN 触发，THEN naming_state→PROMPTED + naming_prompt_triggered 信号发射
- [x] **AC-8**: GIVEN naming_state=PROMPTED + 玩家提交有效名字"小云"，WHEN submit_partner_name("小云")，THEN name="小云" + naming_done=true + naming_state→COMPLETED + naming_completed 信号发射。命名后不再触发
- [x] **AC-9**: GIVEN naming_state=COMPLETED + 玩家尝试再次 submit_partner_name("新名")，WHEN 调用，THEN 拒绝——返回 {accepted: false, error: "naming_completed"}。不可改名
- [x] **AC-10**: GIVEN naming_state=PROMPTED + 玩家提交 "" 或 "   "，WHEN submit_partner_name()，THEN 拒绝 {accepted: false, error: "name_empty"}。naming_skip_count 不增加。naming_state 保持 PROMPTED

### Naming Skip & Default Name

- [x] **AC-11**: GIVEN naming_state=PROMPTED + 玩家点击"稍后"，WHEN skip_naming()，THEN naming_skip_count += 1 + naming_state→PENDING。下次归港再提示
- [x] **AC-12**: GIVEN naming_skip_count=2 + 玩家再次 skip，WHEN skip_naming()，THEN naming_skip_count=3 + name="那只猫" + naming_done=true + naming_state→COMPLETED。静默锁定——无通知弹窗
- [x] **AC-13**: GIVEN 默认名已锁定，WHEN 后续 player_returned_to_hub 触发，THEN naming UI 永不打开。COMPLETED 是终态

### Name Validation

- [x] **AC-14**: GIVEN 玩家提交 12 字符的名字"超长的猫咪名字测试一下"，WHEN submit_partner_name()，THEN 安全截断为 8 字符。主验证在 UI 层——这里是安全网
- [x] **AC-15**: GIVEN 名字已设置 + 存档→读档，WHEN 恢复，THEN 名字不变。无 rename 代码路径

### Nest Accumulation

- [x] **AC-16**: GIVEN nest_state=EMPTY + nest_items=[] + 首次成功嗅辨，WHEN _accumulate_nest_item()，THEN nest_items=[0] + nest_state=FIRST。第 0 件物件：旧船帆碎布
- [x] **AC-17**: GIVEN nest_items=[0] + 第 2 次嗅辨，WHEN _accumulate_nest_item()，THEN nest_items=[0,1] + nest_state=ACCUMULATING。第 1 件：锈蚀的测风链环
- [x] **AC-18**: GIVEN nest_items=[0,1] + 第 3 次嗅辨，WHEN 累积，THEN nest_items=[0,1,2] + nest_state=ACCUMULATING。第 2 件：玩家绳头
- [x] **AC-19**: GIVEN nest_items=[0,1,2] + 第 4 次嗅辨，WHEN 累积，THEN nest_items=[0,1,2,3] + nest_state=FULL。第 3 件：空港徽章残片
- [x] **AC-20**: GIVEN nest_state=FULL + nest_items.size()=4 + 第 5 次嗅辨，WHEN _accumulate_nest_item()，THEN nest_items 保持 [0,1,2,3]。静默跳过——不追加、不报错

### Nest Invariants

- [x] **AC-21**: GIVEN nest_items 已累积 N 件 (N>0)，WHEN 任何操作后检查，THEN nest_items 前 N 件永远匹配静态清单 [0..N-1]。顺序固定
- [x] **AC-22**: GIVEN 任意操作（重复嗅辨、快照往返、Hub 状态转换），WHEN 检查，THEN nest_items.size() 永不低于之前的值。单调不可逆
- [x] **AC-23**: GIVEN 代码库搜索，WHEN 查找，THEN 不存在 nest_items 的删除/清空/重排操作。G.2 不可逆硬约束
...
**Deviations**: Story metadata corrected from `TR-partner-003` to `TR-partner-002` to match the current TR registry naming/nest requirement.
**Test Evidence**: Logic — `tests/unit/partner-relationships/NamingNestTest.csproj` passes 23/23 checks. Story 001 and Story 002 regressions pass.
**Code Review**: Complete — initial public nest mutation concern fixed before closure; nest accumulation is private and reachable through successful `ScoutSniff`.
