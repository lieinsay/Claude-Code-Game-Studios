...
| 字段 | 内容 |
| --- | --- |
| 独立 UI 场景 / 组件 | 目标 `.tscn`、独立组件、注册表条目或 `N/A true`。 |
| 配套脚本 / presenter | 独立 runtime / presenter / controller 文件，或明确 `N/A true`。 |
| 资产组 | 本 UI 专属或共享的一组图标、字体、音频、样式资源。 |
| 装配入口 | 哪个场景、autoload、UIManager 或 shell 只负责引用 / 挂载它。 |
| 禁止混入位置 | 不允许把 UI 本体散落写入的旧 UI 容器、大脚本或临时节点。 |
