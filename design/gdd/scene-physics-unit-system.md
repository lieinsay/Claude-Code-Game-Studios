...
`单位原型` 是可复用的场景单位定义。它声明单位身份、分类、碰撞、遮挡层、尺度规则、特殊表面或物理行为、允许出现的场景、来源 GDD、是否允许 UI 证据等稳定规则。原型做好后，后续场景可以复用它，而不需要重新解释同一种墙、门、舵台、货箱、玻璃或角色的物理读法。
...
| 原型分类 | 含义 | 示例 | 额外要求 |
| --- | --- | --- | --- |
| `dynamic_entity` | 会移动、改变运行时状态、作为 actor 接收交互，或需要同步领域/存档状态的单位 | 玩家标记、NPC、敌人、可推动箱、移动平台 | 必须声明状态来源、行为标签、恢复规则和领域 owner |
| `fixed_scene_object` | 主要作为稳定空间结构、地标、门、通道、遮挡、特殊表面或交互锚点存在的单位 | 舱壁、舵台、货箱、门槛、玻璃、残骸、摊位、前景墙 | 必须声明碰撞/遮挡/尺度、是否可交互、资产替换约束 |
...
5. **Unit Catalog**: 玩家、NPC、道具、阻挡物、可推动物、特殊单位列表。
