...
> **Platform Pivot Note**: ADR-0019 supersedes this GDD's original Web-first implementation constraints. Active MVP work targets desktop Godot 4.6.2 .NET/C#; browser lifecycle, custom HTML shell, JavaScriptBridge, IndexedDB, BFCache, `pagehide`, and Web export requirements below are historical unless explicitly rephrased here as desktop focus / pause / quit requirements.

## Overview

`平台与会话壳` 是《云海织航》的桌面入口与会话生命周期系统。它负责把玩家从桌面应用启动带入一个可恢复、可听见、可操作、可继续的游戏会话：加载游戏、展示开始/继续入口、处理首次音频激活、建立键鼠输入入口、在窗口失焦、暂停、退出或恢复时保护会话状态，并在玩家返回时清楚地告诉他们当前能继续什么。它不是存档系统、UI/HUD 系统或具体玩法系统，但它为这些系统提供可靠的外层节奏，确保玩家在桌面 MVP 中不会因为启动失败、窗口焦点变化、音频策略或恢复不清而失去“飞艇是家”的安全感。

技术约束在本 GDD 中只作为设计约束出现：Godot 4.6.2 .NET 桌面运行、C#、键盘/鼠标、音频需用户手势或设置确认、窗口失焦 / 最小化 / 退出请求可能暂停会话，首屏加载和恢复反馈必须可理解。具体 Autoload、SceneTree、C# 信号绑定、资源加载或存档 API 方案由架构、ADR 和 Control Manifest 约束。

## Player Fantasy

玩家不会把 `平台与会话壳` 当作一个可游玩的系统来感知，但它应该让玩家相信：即使暂时离开，云海中的家与航程也会安稳地留在原处，等他们回来继续。

这个系统支撑的是一种低调的安全感。玩家退出应用、切去别的窗口、最小化游戏，或在桌面窗口恢复焦点后重新进入游戏时，不应该觉得自己被抛回一个冷冰冰的启动流程，也不应该焦虑进度、音频、输入或当前目标是否还可靠。理想体验是：点下“继续”后，飞艇、航线计划和上次的照料节奏被清楚接回，像重新踏回熟悉的甲板。
...
1. `平台与会话壳` 只负责桌面应用的进入、继续、音频激活、焦点恢复、暂停/退出确认与会话生命周期编排。MVP 通过 Godot .NET / C# 接入窗口焦点、暂停、退出请求、可信输入和存储能力探测；存档数据、存储介质、manifest pointer、promotion 和具体 codec 由 `本地存档与世界状态持久化` 与 ADR-0019 后续实现决定。
...
6. 窗口失焦、最小化、系统暂停或退出确认期间，壳层默认进入后台挂起或软暂停。后台状态下，输入不得进入玩法层，游戏不得假装仍在正常交互。
7. 桌面窗口恢复可交互后，必须先确认窗口焦点与输入可用，再做会话恢复检查，再恢复 UI focus。第一次返回输入只用于重新激活或确认，并在同一手势中尝试音频恢复；该输入不得同时触发普通玩法动作。
...
11. 壳层必须维护一个会话连续性不变量：玩家退出、崩溃后重启、窗口恢复或重新进入后，只能恢复到存档系统确认的最近安全继续点，或回到清楚说明不可继续原因的壳层安全状态；不得恢复到未经验证的半初始化状态。
12. MVP 允许 `ephemeral_session_allowed = true`。当桌面存储路径不可用、权限不足或写入被阻止时，玩家可以开始临时会话，但壳层必须在进入前明确提示“本次进度不会保存”，并且不得显示或生成持久化 `Continue`。
...
| `BackgroundSuspended` | 窗口失焦、最小化、系统暂停或退出确认期间，壳层冻结会话推进并保留挂起 token | None | 窗口恢复可交互，或必须回到标题/恢复错误态 |
| `ResumePending` | 窗口已恢复，等待恢复检查、焦点重获和明确重新激活 | Confirm/click/key reactivation, Return Title | 恢复成功、恢复失败或退出 |
...
| `Loading -> FatalBlocked` | 核心资源缺失、内容域 `VERSION_INCOMPATIBLE`、构建不兼容或必需桌面运行能力不足 |
...
| `SessionActive -> BackgroundSuspended` | 窗口失焦、最小化、系统暂停或退出确认；壳层创建挂起 token，并只请求存档系统执行已准备的轻量 marker，不请求完整 safe checkpoint |
| `BackgroundSuspended -> ResumePending` | 窗口恢复可见且可接收可信输入 |
| `ResumePending -> SessionActive` | 恢复检查通过，玩家完成明确重新激活 |
| `ResumePending -> RecoveryRequired` | 恢复检查失败 |
| `Any -> FatalBlocked` | 不可恢复平台错误或核心初始化失败 |

加载子阶段不升级为主状态，但必须用于诊断和重试分类：`BaseBoot`、`ContentDomainCheck`、`StorageCapabilityCheck`、`SessionMetadataCheck`、`EntryRenderReady`。每次失败报告必须包含失败子阶段、失败类型、是否可重试、窗口焦点/可交互状态和是否存在有效继续点。

桌面生命周期事件必须幂等处理：同一失焦、最小化、暂停、退出确认或恢复序列只能生成一个挂起 token；重复 focus / restore 事件只能刷新现有 token 状态，不能创建第二个恢复流程。focus / trusted input 到达顺序不可靠时，以挂起 token 的当前阶段为准：未进入 `ResumePending` 前丢弃玩法输入，进入 `ResumePending` 后第一下可信输入只确认恢复。

桌面生命周期归一化表：

| Desktop Signal | Shell Interpretation | Required Handling |
|---|---|---|
| Window focus lost | 窗口不可交互或玩家离开 | 进入或保持 `BackgroundSuspended`，只请求已准备 marker，不开放玩法输入，不启动完整保存。 |
| Window minimized / OS pause | 应用可能暂停或长期不可见 | 进入 `BackgroundSuspended`，冻结玩法输入和会话推进。 |
| Quit requested | 玩家或系统请求退出 | 只请求已准备 marker；完整保存必须已经在安全边界完成，退出路径不得承诺新 checkpoint 一定完成。 |
| Window focus restored | 窗口恢复可交互 | 不直接回到 `SessionActive`；必须进入 `ResumePending`，重新确认输入焦点、内容域状态和存档系统的最近安全点。 |
| Process restart after crash / forced close | 旧内存不可信 | 重新进入 `Booting` / `Loading`，由存档系统重新读回验证。 |
...
| Desktop platform adapter | none at design level | window focus changed, pause / quit requested, trusted input received, viewport resized, storage capability | 拥有桌面生命周期和能力信号接入；具体 Godot .NET / C# 绑定留给 ADR 和 Control Manifest |
...
| `foreground_visible` | `Fv` | bool | true/false | 桌面窗口是否可见 / 未最小化 |
| `foreground_interactive` | `Fi` | bool | true/false | 桌面窗口是否拥有焦点并可接收可信输入 |
...
**Example:** 如果会话处于 `Suspended` 且桌面窗口已回到前台并获得焦点，则输出 `Reacquire`，表示第一次输入只用于重新激活，不能触发普通玩法动作。
...
| `foreground_visible` | `Fv` | bool | true/false | 桌面窗口是否已回到可见状态 |
| `foreground_interactive` | `Fi` | bool | true/false | 桌面窗口是否已可接收可信输入 |
...
**Example:** 桌面窗口恢复焦点后，如果玩家尚未明确点击或按键重新激活，则输出 false。
...
| `storage_capability` | `P` | enum | `PersistentAvailable` / `WriteLocked` / `EphemeralOnly` | 当前桌面本地存储能力；旧档可读但不能可靠写入时为 `WriteLocked`，旧档也不可读或策略强制临时时为 `EphemeralOnly`。 |
...
- **If 窗口失焦、最小化、系统暂停或退出确认发生**: 立即进入 `BackgroundSuspended`，冻结玩法输入和会话推进，创建或复用挂起 token，并只请求存档系统执行已准备 marker；不得触发关卡推进、资源变化、完整序列化、readback、checksum、迁移、备份提升或会话切换。
- **If 桌面窗口从隐藏/失焦/暂停恢复但玩家尚未明确重新激活**: 保持 `ResumePending`，不接受玩法输入、不消耗资源、不播放推进音效。
...
- **If 应用重启、强制关闭后重新打开、进程崩溃恢复或系统恢复**: 不得复用未验证的旧内存状态。壳层必须重新进入 `Booting` / `Loading`，读取存档系统确认的最近安全继续点；若无法确认，则显示 `RecoveryRequired` 或 `PreservedLocked`，不得直接进入玩法。
- **If 桌面本地存储不可用或写入被阻止**: 视为软失败，允许当前临时会话继续，但必须在进入前显示无保存提示，禁用持久化继续点写入，并确保 `Continue` 不会伪装成可用。
- **If 桌面本地存储能力检测显示旧档可读但新写入被阻止**: MVP 中归一化为 `storage_capability = WriteLocked`；壳层可呈现已验证旧 Continue，但必须显示新进度无法可靠保存的受控路径。
- **If 桌面本地存储能力检测显示旧档读取也不可靠、底层 API 不可用或平台策略强制临时会话**: MVP 中归一化为 `storage_capability = EphemeralOnly`；不得保留一个未处理的 `Unavailable` 分支。
...
`平台与会话壳` 是 Foundation 层系统。它没有更早的玩法依赖，但它依赖桌面 Godot .NET 运行环境提供基础窗口可见性、输入焦点、音频激活和存储能力信号。它同时是多个后续系统的入口门禁：后续系统不能绕过壳层直接进入可操作玩法。

| Dependency | Type | Direction | Interface / Contract |
|---|---|---|---|
| Godot 4.6.2 .NET desktop runtime | Hard | Platform -> Shell | 提供桌面运行、基础渲染、键鼠输入、窗口/焦点/可见性变化、音频输出和本地存储能力信号。 |
| Windows desktop environment | Hard | Platform -> Shell | Windows 桌面优先；必须按窗口失焦、最小化、暂停/退出请求、焦点恢复和本地存储可用性限制设计。 |
...
| Desktop platform adapter | Hard | Platform -> Shell | 提供 focus/blur、pause/resume、quit request、可信输入、viewport resize、storage capability 和 process restart 线索；生命周期和能力信号由壳层/平台适配层归一化，具体接入方式留给 ADR / Control Manifest。 |

硬依赖：

- Godot .NET desktop runtime 与必要桌面窗口/输入能力必须存在，否则进入 `FatalBlocked`。
...
| `audio_soft_fail_allows_play` | true | true | false 会因为音频初始化/设备异常阻断游戏 | 不适用；该值应固定为 true |
...
| `safe_close_request_window_ms` | best effort only, no guarantee | best effort | 假设一定保存会造成误导 | 过度等待可能被 OS 或用户强制关闭中断 |
...
`平台与会话壳` 不要求独立资产规格，但它必须遵守《云海织航》的“航路修复主义”视觉语言：入口、加载、继续、恢复和错误状态应像可靠的航务/港口界面，而不是通用启动器。
...
| `HardFail` | 音频系统初始化破坏核心运行或桌面平台能力异常 | 进入安全错误态；不得用刺耳音效提示 |
...
- **GIVEN** 游戏处于 `SessionActive`，**WHEN** 窗口失焦、最小化、系统暂停或退出请求发生，**THEN** 游戏进入 `BackgroundSuspended`，玩法输入停止，世界推进停止。
- **GIVEN** 游戏处于 `SessionActive`，**WHEN** 窗口失焦、最小化、退出请求或进程重启线索发生，**THEN** 壳层只请求已准备 marker，不得请求完整 safe checkpoint，并且后续重新进入时必须从 `Booting` / `Loading` 重新验证继续点。
- **GIVEN** 桌面窗口或进程从暂停/恢复路径回来，**WHEN** 窗口回到前台，**THEN** 壳层必须进入 `ResumePending` 并重新执行恢复检查，不得直接回到 `SessionActive`。
- **GIVEN** 桌面退出请求或进程终止信号触发，**WHEN** 壳层处理关闭前信号，**THEN** 不得启动新序列化、迁移、备份提升或阻塞式保存。
- **GIVEN** 游戏从后台返回并进入 `ResumePending`，**WHEN** 桌面窗口已可见/有焦点但玩家尚未显式重新激活，**THEN** 玩法输入仍然被阻断，第一下输入只用于重新激活，不得触发普通玩法动作。
- **GIVEN** 游戏处于 `ResumePending`，**WHEN** 玩家按下键盘或鼠标作为恢复操作，**THEN** 该输入只能用于重新激活和同手势音频恢复，不能同时触发任何普通玩法动作。
- **GIVEN** 游戏处于 `ResumePending`，**WHEN** 桌面窗口已回前台、挂起 token 有效、玩家完成明确重新激活、焦点恢复且内容域可用，**THEN** 状态转入 `SessionActive`。
...
- **GIVEN** 核心资源缺失、构建不兼容、缺少必需 Godot .NET desktop runtime、缺少可用渲染上下文或内容域版本不兼容，**WHEN** 加载失败，**THEN** 状态转入 `FatalBlocked`，只保留安全错误态与重试/返回类入口，不得进入玩法。
...
- **GIVEN** 游戏处于 `BackgroundSuspended`，**WHEN** 桌面窗口恢复可见且可交互，**THEN** 状态转入 `ResumePending`，但仍不得接受普通玩法输入。
...
- **GIVEN** 会话状态为 `Active`、桌面窗口在前台、输入焦点正常且无模态阻挡，**WHEN** 计算 `input_gate`，**THEN** 结果必须是 `Open`。
- **GIVEN** 会话状态为 `Suspended` 且桌面窗口已回前台，**WHEN** 计算 `input_gate`，**THEN** 结果必须是 `Reacquire`。
...
- **GIVEN** 桌面本地存储不可用或写入被阻止，**WHEN** 玩家选择 `Start`，**THEN** 壳层必须先显示临时会话无保存提示；玩家确认后可进入临时会话，但不得生成持久化继续点。
- **GIVEN** 桌面存储 API、后端 probe、可用空间、写入 roundtrip、平台策略或旧档读取状态发生变化，**WHEN** 壳层收到桌面平台适配层信号，**THEN** 壳层只把 raw `persistence_probe` 传给存档系统；`storage_capability` 必须使用存档系统返回的 `PersistentAvailable` / `WriteLocked` / `EphemeralOnly`，壳层不得本地计算。
...
- **Owner: Technical Direction**; **Target: ADR / Control Manifest**; 已定设计约束为 Godot .NET / C# 平台适配层捕获桌面窗口焦点、暂停/恢复、退出请求和存储能力信号；ADR / Control Manifest 决定具体绑定、初始化顺序和测试桩实现。
