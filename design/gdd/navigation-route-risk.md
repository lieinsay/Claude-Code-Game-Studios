...
**明确不在 MVP 范围内**：航行表现层的具体视觉效果和动画（由 UI/HUD/航图界面 #16 和反馈/特效/音频 #17 消费本系统事件后实现）；探索点的具体内容生成（由探索/搜撤场景 #11 消费 EncounterContext 后实现）；战斗的具体规则和结算（由战斗与威胁处理 #12 实现）；NPC 船队的具体贸易逻辑和路线生成（由 #14 定义，本系统仅提供风险解析服务）；连续操控模型和实时飞艇驾驶（本系统是离散风险解析，不是物理飞行模拟）。
...
| 航线 ID | 距离带 | 航行时长 | 遭遇检查次数 | 可见风险标签 | 隐藏风险标签 |
|--------|--------|---------|------------|------------|------------|
| `route.sky-reef-arc-01` | short | 60s | 5 次 | `safe` | 无 |
| `route.storm-cut-01` | medium | 120s | 10 次 | `storm` | `low-visibility` |
