...
7. 航行遭遇战第一版不包含主动开火。对抗问题必须通过规避、甩脱、干扰、硬抗或撤退处理；主动武器、瞄准、射击、击毁敌人属于后续开发范围。

**航行问题与非射击遭遇**

16. 航行问题分为环境问题和对抗问题。环境问题考验航路判断；对抗问题考验移动中的应变，但二者都必须服务航行，不应把场景切换成独立战斗房间。

| 问题类型 | 玩家可读信号 | 第一版解决方式 | 失败或代价 |
| --- | --- | --- | --- |
| 浓雾带 | 远景消失、航标弱化、云层压近 | 减速穿越、绕行、消耗侦察扫描 | 迷航、错过目标、航程变长 |
| 漂浮残骸群 | 近景残骸快速接近，存在可穿缝隙 | 转向穿缝、后退重找角度、低速通过 | 船体损伤、航向偏移 |
| 风暴边缘 | 远处雷光、云层翻涌、仪表警告 | 远离风暴、贴边抢时间、撤退 | 模块损伤、船体损伤、风险升级 |
| 失准航标 | 航标闪烁或指向矛盾 | 观察多个航标交叉确认、使用情报/航图校准 | 进入危险支线、绕远 |
| 空盗拦截 | 侧后方出现追击船影，航标/警报转为敌对状态 | 躲进云层、急转甩脱、穿过残骸区阻挡追击、撤退 | 船体损伤、资源损失、被迫改道 |
| 失控无人浮标 | 航道中出现旋转扫描/锁定装置 | 绕开扫描区、等待间隙、用航图/侦察模块校准关闭 | 侦察短暂失效、模块短路、航程变长 |

17. 对抗问题的默认解法不应奖励“击毁敌人”，而应奖励读航路和控船：利用云层断锁、利用残骸阻挡追击、等待巡逻窗口、短暂关闭信号、牺牲速度换安全，或在风险过高时撤退。
