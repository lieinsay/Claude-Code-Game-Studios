...
本系统适用于所有当前和后续可进入场景：标题/声场确认、玻璃港停泊浮岛、云织号船内、航图桌、雾海搜撤、世界修复/解锁点、旧集市交易、未来村镇/空港扩展。任何场景进入实现前必须拥有场景规格；任何场景进入 release gate 前必须通过双审：Codex 设计/实现审查 + 用户人工体验审查。
...
2. **Scene Physics Contract**: 链接或嵌入 `场景单位物理设计` 要求的物理契约，包含场景物理类型、移动平面、单位目录、碰撞、遮挡、尺度、特殊表面、动态行为和恢复规则。
...
| --- | --- | --- |
| 标题/声场确认 | UI 可运行，正式主题资产缺失 | 首屏必须传达《云海织航》主题，不只是壳层按钮 |
| 玻璃港停泊浮岛 | 灰盒可视化，人工 QA 曾读不出岛/码头 | 规格必须定义岛、码头、云织号、登船路径、外部 Chart 禁用反馈 |
| 云织号船内 | 灰盒分区存在，仍需正式空间语言 | 规格必须定义驾驶舱、货舱、轮机间、走廊、生活痕迹和状态变体 |
| 航图桌 | 最新提交补灰盒场景面 | 规格必须定义桌面、羊皮纸、路线节点、风险标记、确认门和焦点隔离 |
| 雾海搜撤 | 灰盒探索地和三段搜索存在 | 规格必须定义残骸、线索、搜索节拍、威胁区、返航船和撤离读法 |
| 世界修复/解锁点 | 系统存在，可进入场景缺失 | 必须新增灯塔/设施修复场景规格和资产门禁 |
| 旧集市交易 | 系统存在，可进入场景缺失 | 必须新增旧集市、NPC、摊位、商品、购买反馈场景规格 |
