...
| # | 屏幕 ID | 名称 | 类型 | 所属系统 | 打开触发 |
|---|---------|------|------|---------|---------|
| S1 | `hud_hub` | Hub 常驻 HUD | HUD 覆盖层 | #7 | Hub 场景激活时自动显示 |
| S2 | `panel_station` | 站点交互面板 | 非模态/模态混合 | #7 | 玩家对站点按 Use |
| S3 | `modal_departure` | 出航确认对话框 | 模态 | #7/#9 | 舱门 Use（Mode A）/ 舵轮 Use（Mode B） |
| S4 | `screen_chart` | 航图屏幕 | 全屏 | #9 | 出航确认后 或 M 键快捷打开 |
| S5 | `hud_exploration` | 探索场景 HUD | HUD 覆盖层 | #11 | EXPLORING 阶段激活 |
| S6a | `modal_capacity` | 容量取舍面板 | 模态 | #11 | 随身物品栏满格时捡到新物品 |
| S6b | `overlay_extraction` | 撤离读条 | 半模态覆盖 | #11 | 玩家在撤离锚点确认撤离 |
| S6c | `modal_settlement` | 结算摘要 | 模态 | #11 | 撤离完成 / 返回 Hub |
| S7 | `modal_threat` | 战斗威胁决策面板 | 模态（覆盖级） | #12 | threat 触发 |
| S8 | `panel_repair` | 修复交互面板 | 模态 | #13 | 玩家对修复节点按 Use |
| S9 | `modal_market` | 摊位购买界面 | 模态 | #14 | 玩家对摊位按 Use |
| S10 | `modal_naming` | 伙伴命名模态 | 模态（阻断级） | #15 | naming_prompt_eligibility=true 且到达序列中 |
| S11 | `panel_sniff` | 伙伴嗅辨面板 | 非模态 | #15 | 玩家对伙伴驻点按 Use（嗅辨动词） |
| S12 | `panel_storage` | 仓库/货舱整理界面 | 非模态 | #5 | 玩家对仓库/货舱锚点按 Use |
