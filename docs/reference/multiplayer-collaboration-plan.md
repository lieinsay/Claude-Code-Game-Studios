...
> **文档索引**: `docs/document-index.md` 已于 2026-05-22 同步到 #18、Polish Story 001-007 完成状态。Epic #1-#18 Complete + Sprint 003 domain-backed playable evidence 已通过 Production → Polish gate；fresh perf probe 已修复并通过，Navigation/Exploration runtime hardening、authored route/search content slice、authored content validation guard、route/search ID migration guard、spatial scene separation/walkable prototype、cross-launch persistence trust、真实 preflight、dynamic Exploration semantics、windowed screenshot evidence 已完成，后续进入 full room interiors / save-slot UX / art-audio treatment 的普通 Polish backlog，其中内容节奏、美术/音频口味和长时 playtest 需要人工判断。
...
D-C: Chart / Exploration route authority + QA smoke
  ...
  ☑ [Polish Story 007] 将 canonical progress manifest 写入 user:// 并验证 SessionShell 重启后加载恢复
