...
- 当前 blocker: 无 Production 硬阻塞；#18 implementation、fresh perf probe、Navigation/Exploration runtime hardening、fixture JSON 化、真实 preflight、windowed visual capture、richer Exploration scene semantics 均已完成。剩余风险转为最终内容生产与美术/音频处理的普通 Polish backlog。
...
| 优先级 | 行动 | 依赖 | 预计 |
|--------|------|------|------|
| **P0** | Final authored route/search content backlog slice | Polish Story 001/002 ✅ | Next |
| **P1** | Final art/audio treatment for onboarding/runtime hints | #18 implementation slice + Story 002 visual semantics ✅ | Polish |
| **P2** | Additional windowed capture/video if required | Windowed screenshots PASS ✅ | Polish |

> 关键建议: Polish Story 001/002 已把当前 adapter fixture 的运行时权威扩展到 NavigationManager EncounterContext 与 ExplorationManager search/threat contract，并完成 fixture JSON 化、真实 preflight、动态探索场景语义与 windowed screenshot 证据。下一步不要宣布 Release readiness；继续用小步 Polish backlog 推进最终 route/search 内容生产或美术/音频处理。
