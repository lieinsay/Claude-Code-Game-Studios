...
> 当前状态: **Polish | Phase 0-A-F/#18 代码与文档证据完成 | Sprint 003 domain-backed playable slice PASS | Production → Polish PASS WITH CONDITIONS | #18 Onboarding 5 Stories Complete | Polish Story 001-006 Complete**
...
- 活跃任务: Polish backlog follow-ups after Story 006 spatial scene separation and walkable prototype。
- 已接受证据: Sprint 002 灰盒 playable slice 恢复通过，人工可完成 Hub -> Chart -> Exploration -> Return + save/load restore。
- 当前 blocker: 无 Production 硬阻塞；#18 implementation、fresh perf probe、Navigation/Exploration runtime hardening、authored route/search content slice、authored content validation guard、route/search ID migration guard、spatial scene separation/walkable prototype、真实 preflight、windowed visual capture、richer Exploration scene semantics 均已完成。剩余风险转为 cross-launch persistence trust、full room interiors、美术/音频处理和长时 playtest 的普通 Polish backlog，需要人工设计/QA 判断。
...
| 优先级 | 行动 | 依赖 | 预计 |
|--------|------|------|------|
| **P0** | Cross-launch persistence trust verification/fix | Human QA found close/relaunch restore risk | Next |
| **P1** | Full room interiors and richer walkable ship/island spaces | Polish Story 006 spatial prototype ✅ | Needs design input |
...
> 关键建议: Polish Story 001-006 已把当前 adapter fixture 的运行时权威扩展到 NavigationManager EncounterContext 与 ExplorationManager search/threat contract，并完成 authored content slice、轻量 content validation guard、route/search ID migration guard、spatial scene separation/walkable prototype、真实 preflight、动态探索场景语义与 windowed screenshot 证据。下一步不要宣布 Release readiness；优先验证 cross-launch persistence trust，或继续根据人工设计输入推进 full room interiors / final art-audio。
